...
interface Outpass {
  ...
  inDate: string;
  inTime: string;
  status: string; // Added this, as it's likely needed
  studentId: {
    name: string;
    department: string;
    year: string;
...
export default function AdvisorDashboard() {
  ...
  // Fetch advisor ID and all data on load
  useEffect(() => {
  ...
  async function fetchAllData(id: string) {
    ...
    setError('');
    try {
      // Assumes you have two separate endpoints
      const pendingRes = await api.get(`/advisor/pending/${id}`);
      const historyRes = await api.get(`/advisor/history/${id}`); // Assumed endpoint
  ...
  const handleAction = async (
    ...
    try {
      await api.put(`/advisor/${action}/${outpassId}`);
      // Success! Refetch both lists to update UI
      fetchAllData(advisorId);
    } catch {
